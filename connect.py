...
def log_record(username, host):
    ...
    pid = os.getpid()
    ip_list = []
    remote_ip = os.popen("who | grep '%s'| awk '{print $5}'|tr -d '()'" %username).readlines()
    for ip in remote_ip:
...
def print_prompt():
    ...
    2) Type \033[34mP/p\033[0m To Print The Servers You Available.
    3) Type \033[34mG/g\033[0m To Print The Server Groups You Available.
    4) Type \033[34mG/g(1-N)\033[0m To Print The Server Group Hosts You Available.
    5) Type \033[34mE/e\033[0m To Execute Command On Several Servers.
...
def connect(username, password, host, port, login_name):
    ...
    try:
     ...
     #   print username,password,host,port,login_name
